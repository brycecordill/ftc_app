...
@Autonomous(name = "Auto Red PERP Sensor", group = "3650 New")
public class Auto_Red_Perp_Sensor extends LinearOpMode{
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (col == 1){
            setBothEncoder(-430); //Good
            setBothPower(.4);
        }
        else if (col == 0){
            setBothEncoder(670);  //Good
            setBothPower(.4);
        }
        else if (col == -1){
            setBothEncoder(1570); //Good
            setBothPower(.4);
        }
        else {
            //Go for center
